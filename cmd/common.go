...
func templateTfVars(config *awsbi.Config) error {
...
func showModulePlan(config *awsbi.Config, state *st.State) error {
	...
	futureState.AwsBI.Config = config
	futureState.AwsBI.Status = st.Applied
...
func updateStateAfterDestroy(state *st.State) *st.State {
	...
	state.AwsBI.Output = nil
	state.AwsBI.Status = st.Destroyed
	return state
}
