...
ARG ARG_GO_MODULE_NAME="github.com/epiphany-platform/m-aws-basic-infrastructure"
...
RUN CGO_ENABLED=0 GOOS=linux go build -ldflags="-s -w $(govvv -flags -pkg $GO_MODULE_NAME/cmd -version $M_VERSION)" -x -o /m-aws-basic-infrastructure $GO_MODULE_NAME
...
WORKDIR /workdir
ENTRYPOINT ["/workdir/m-aws-basic-infrastructure"]

COPY --from=initializer /resources/ /resources/
COPY --from=builder /m-aws-basic-infrastructure /workdir
